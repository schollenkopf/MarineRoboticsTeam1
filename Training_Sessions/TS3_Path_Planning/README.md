# Training Sessions 3: Path Planning

This session will give a gentle introduction to the different planning algorithms that are available to us.

Planning is essential both for offline planning of the intended path we want the robot to follow, but even more essential for online replanning on the robot, to avoid colliding with obstacles.

The implemented algorithms are forked from: https://github.com/zhm-real/path-planning-algorithms


## Initial Setup
> NOTE: This exercise is meant to be run in the Docker container, but you are free to run it how you see fit, but you will be on your own when encountering issues.

First ensure that you have a conda environment active, so open a terminal and run the `./scripts/activate_conda.sh` script.
You can also do it manually by typing

    source /home/ubuntu/miniconda3/bin/activate

You can check if the conda environment is active by making sure you see the `(base) ubuntu@....: $` like shown below

![](media/conda_base_terminal.png)

Now install `jupyterlab` by running
...
Firefox should open automatically to a page looking like this:

![](./media/jupyter_startup.png)

Now navigate to the folder `34763-autonomous-marine-robotics/Training Sessions/TS3 Path Planning` using the navigation pane on the left side (as shown in the image).

> TIP:
> You can render this readme file directly in jupyterlab as well, open the file, and move it to the right side of the , and right-click in the content and select `Show Markdown Preview`, now right-click the markdown tab and select `New view for Markdown File` - This way you can follow along in the same browser tab!


## Exercise 1

Run through the notebook `exe1_search_based.ipynb`.
This will give you an introduction to some of the search based planning algorithms discussed in the lecture.

## Exercise 2

Run through the notebook `exe2_sampling_based.ipynb`.


## Exercise 3

Run through the notebook `exe3_sampling_based_seachart.ipynb`.


<!-- 
Forked from: https://github.com/zhm-real/path-planning-algorithms
